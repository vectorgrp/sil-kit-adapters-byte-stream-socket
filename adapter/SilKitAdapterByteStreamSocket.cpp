...
const std::string adapters::defaultParticipantName =
    "SilKitAdapterByteStreamSocket";

void PrintVersion() {
  std::cout << "SIL Kit Adapter Byte Stream Socket - version: "
            << SILKIT_ADAPTER_VERSION << std::endl;
}
...
int main(int argc, char **argv) {
  ...
  try {
    throwInvalidCliIf(thereAreUnknownArguments(
        argc, argv,
        {&bytestreamArg, &regUriArg, &logLevelArg, &participantNameArg,
         &configurationArg, &unixBytestreamArg},
        {&helpArg}));
    ...
    const auto participant =
        CreateParticipant(argc, argv, logger, &participantName,
                          &lifecycleService, &runningStatePromise);
    ...
    foreachArgDo(argc, argv, bytestreamArg, [&](char *arg) -> void {
      ...
      transmitters.emplace_back(SocketToBytesPubSubAdapter::parseArgument(
          arg, alreadyProvidedSockets, participantName, ioContext,
          participant.get(), logger, false));
    });
    ...
    Stop(ioContext, ioContextThread, *logger, &runningStatePromise,
         lifecycleService, &finalStateFuture);
